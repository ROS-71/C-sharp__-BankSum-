...
namespace TextBoxAnimation
	...
	public class ClassTBAnimation : Control
	{
		...
		// Описываем визуальные параметры нашего TextBox
		public ClassTBAnimation()
			{
		...
		// Рисование в методе OnPaint - он вызываетя всегда когда необьходимо переерисовать объект
		protected override void OnPaint(PaintEventArgs e)
			{

			// Базовый метод OnPaint - чтобы он вызывал обработчики событий, подписанные на событие Paint
			// Если не вызывать бызовый метод, то некоторые аспекты не будут рисоваться и обработчик событий не будет вызван
			base.OnPaint(e);

			// Объявление объекта Graphics - с его помощью выполняется вся визуализация графического интерфейса
			// он содержит все необходимые методы для рисования
			// Для определения Graphics обращаемся к аргументу текущего метода и указываем его свойсво Graphics таким образом задаём его область рисования
			Graphics graph = e.Graphics;
			graph.SmoothingMode = SmoothingMode.HighQuality;			// Указываем качество визуализации - Высокое

			// Вызываем метод Clear - этот метод очищает всю поверхность рисования и заливает её указанным цветом
			// В качестве передаваемого цвета будет BackColor родительского контейнера
			// Таким образом наш контрол сливается с задним фоном
			...
			// Обводка
			// Вызываем метод DrawRectangle - он рисует прямоугольник
			// Нашим цветом будет стандартное свойсво - BackColor
